...
def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
  "SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) AS result_funding
  FROM projects JOIN pledges ON projects.id = pledges.project_id
  GROUP BY projects.funding_goal HAVING result_funding >= 0"
end

def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_summed_amount
  "SELECT users.name, SUM(pledges.amount) FROM pledges JOIN users WHERE users.id = pledges.user_id GROUP BY users.name ORDER BY SUM(pledges.amount);"
end
...
def selects_the_category_names_and_pledge_amounts_of_all_pledges_in_the_music_category
  ...
  # "SELECT projects.category, SUM(pledges.amount) FROM projects = projects.category = "music""  ---> first try
end
